...
sealed class AwsEvents : IProduceDriver
{
    ...
    {
        logger.LogInformation("Putting EventBridge SNS target {TopicTopicName}[{SnsArnValue}]", topic.TopicName,
            snsArn.Value);
        ...
        {
            logger.LogInformation("Target with {TopicTopicName}[{SnsArnValue}] already added", topic.TopicName,
                snsArn.Value);
            return;
        }
        ...
        logger.LogInformation(
            "Completed({ResultHttpStatusCode}): EventBridge SNS target {TopicTopicName}[{SnsArnValue}]",
            result.HttpStatusCode, topic.TopicName, snsArn.Value);
    }
